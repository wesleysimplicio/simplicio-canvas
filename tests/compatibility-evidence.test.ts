...
describe('compatibility evidence metadata', () => {
  it('records scan, graph, diagnostics and screenshot provenance for verified captures', () => {
    ...
    for (const capture of evidence.captures) { expect(capture.scanMs).toBeGreaterThanOrEqual(0); expect(capture.nodes).toBeGreaterThanOrEqual(0); expect(capture.edges).toBeGreaterThanOrEqual(0); expect(capture.flows).toBeGreaterThanOrEqual(0); expect(capture.diagnostics).toBeGreaterThanOrEqual(0); expect(capture.status === 'unsupported' ? capture.reason : capture.memoryMb).toBeTruthy(); expect(capture.screenshot).toMatch(/^assets\/.+\.png$/); expect(JSON.stringify(capture)).not.toMatch(/(?:content|sourceBody|token|password|private[_-]?key)/i) }
  })
  it('keeps the real simplicio-loop capture aligned with the Mapper scan receipt', () => { const loop = evidence.captures.find((capture) => capture.repository === 'wesleysimplicio/simplicio-loop')!; expect(loop).toMatchObject({ mapperVersion: '0.19.0', nodes: 450, edges: 1000, diagnostics: 0, status: 'verified' }) })
