...
For representative public repositories, `npm run scan:mapper:matrix -- --ids=requests,spring-petclinic --output=output/mapper-matrix-evidence.json` performs depth-1, blob-filtered clones into disposable directories and deletes them in `finally`. A verified run recorded Mapper `0.19.0`: Requests (85 files, 9 layers, 6 modules, 807 symbols, 1,000 relationships, 2.392s) and Spring Petclinic (91 files, 10 layers, 5 modules, 161 symbols, 193 relationships, 1.871s). Both completed with zero changed files, artifacts present and zero cache entries.

Add `--determinism` to repeat a scan and compare aggregate counts. The report also includes parent-process RSS before/after/delta; this is explicitly not child-process peak memory. A verified Requests repeat was deterministic (`true`) with 85 files/9 layers/6 modules/807 symbols/1,000 relationships and an RSS delta of -7,962,624 bytes. Peak child memory and screenshots remain future CI work and are not claimed by this report.
