export const ACTIVE_LOCALES = ['en', 'pt-BR', 'es', 'fr', 'de', 'it', 'ja', 'ko', 'zh-CN', 'ru', 'pl', 'tr', 'nl', 'hi', 'ar'] as const
...
const translatedMessages: Partial<Record<Locale, Record<string, string>>> = {
...
// Keep every visible key available in every locale.  The four launch locales
// have curated copy; the remaining locales intentionally use English fallback
// until their reviewed translations land, but they are real selectable locales
// (never disabled controls or missing-key labels).
const messages = Object.fromEntries(ACTIVE_LOCALES.map((locale) => [locale, { ...translatedMessages.en, ...(translatedMessages[locale] ?? {}) }])) as Record<Locale, Record<string, string>>
