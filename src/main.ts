...
let group = new THREE.Group(); scene.add(group); const raycaster = new THREE.Raycaster(); const pointer = new THREE.Vector2(); const meshNodes = new Map<THREE.Object3D, ArchitectureNode>(); const nodeSprites=new Map<THREE.Object3D,THREE.Sprite>(); let dragControls:DragControls|undefined; let analysis: ProjectAnalysis | undefined
...
function renderGraph(graph: ArchitectureGraph, documented: Array<{ from: string; to: string }> = []) {
  scene.remove(group); group = new THREE.Group(); scene.add(group); meshNodes.clear(); nodeSprites.clear(); dragControls?.dispose(); const byId=new Map<string,THREE.Mesh>(); const edgeMeshes:THREE.Mesh[]=[]
  graph.nodes.forEach((node) => { const material=new THREE.MeshStandardMaterial({color:LAYERS[node.layer].color,roughness:.46,metalness:.12});const mesh=new THREE.Mesh(puzzleGeometry(node.kind),material);mesh.rotation.x=-Math.PI/2;mesh.position.set(node.x,1,node.z);mesh.castShadow=true;mesh.receiveShadow=true;mesh.userData.baseY=1;group.add(mesh);meshNodes.set(mesh,node);byId.set(node.id,mesh);const sprite=new THREE.Sprite(new THREE.SpriteMaterial({map:labelTexture(node),transparent:true}));sprite.scale.set(3.4,1.4,1);sprite.position.set(node.x,2.5,node.z);group.add(sprite);nodeSprites.set(mesh,sprite) })
  ...
  dragControls=new DragControls([...meshNodes.keys()],camera,canvas);dragControls.addEventListener('dragstart',()=>{controls.enabled=false});dragControls.addEventListener('drag',(event)=>{const mesh=event.object;mesh.position.y=1;mesh.userData.baseY=1;const sprite=nodeSprites.get(mesh);if(sprite)sprite.position.set(mesh.position.x,2.5,mesh.position.z);edgeMeshes.forEach(refreshEdge)});dragControls.addEventListener('dragend',(event)=>{controls.enabled=true;event.object.position.x=Math.round(event.object.position.x*2)/2;event.object.position.z=Math.round(event.object.position.z*2)/2;edgeMeshes.forEach(refreshEdge)})
...
canvas.addEventListener('click',(e)=>{const rect=canvas.getBoundingClientRect();pointer.set((e.clientX-rect.left)/rect.width*2-1,-(e.clientY-rect.top)/rect.height*2+1);raycaster.setFromCamera(pointer,camera);const hit=raycaster.intersectObjects([...meshNodes.keys()])[0];if(!hit)return;const node=meshNodes.get(hit.object)!;const file=analysis?.files.find(item=>item.path===node.path);const incoming=analysis?.connections.filter(item=>item.target===node.path&&!item.external)??[];const imports=file?.imports??[];const lines=file?sourceLines(file.content):[];const flows=file?.path.match(/readme/i)?analysis?.documentedFlows??[]:[];document.querySelector('#selection')!.innerHTML=`<div class="piece-icon" style="--c:${LAYERS[node.layer].color}">⊕</div><p class="tag">${node.layer} / ${node.kind}${file?` / ${file.language}`:''}</p><h2>${node.name}</h2><code>${node.path}</code>${file?`<dl><dt>LINHAS</dt><dd>${file.lines}</dd><dt>IMPORTS</dt><dd>${imports.length}</dd><dt>IMPORTADO POR</dt><dd>${incoming.length}</dd></dl><p class="subhead">TERMINAL · SOMENTE LEITURA</p><div class="code-terminal"><header><i></i><i></i><i></i><span>${file.language}</span></header><pre>${lines.map(line=>`<span><b>${line.number}</b>${line.text||' '}</span>`).join('')}</pre></div>${flows.length?`<p class="subhead">FLUXOS DOCUMENTADOS</p><div class="imports documented">${flows.map(flow=>`<span>⇢ ${flow.source} <b>→</b> ${flow.target}</span>`).join('')}</div>`:''}<p class="subhead">DEPENDÊNCIAS</p><div class="imports">${imports.length?imports.map(item=>`<span class="${item.external?'external':''}">→ ${item.specifier}<small>${item.resolvedPath??'pacote externo'}</small></span>`).join(''):'<em>nenhuma detectada</em>'}</div>`:`<dl><dt>ENTRADA</dt><dd>${PIECES[node.kind].socket}</dd><dt>SAÍDA</dt><dd>${PIECES[node.kind].tab}</dd></dl>`}`})
const help=document.querySelector<HTMLElement>('#help')!;const toggleHelp=()=>{help.hidden=!help.hidden};document.querySelector('#help-trigger')!.addEventListener('click',toggleHelp);document.querySelector('#help-close')!.addEventListener('click',toggleHelp);addEventListener('keydown',(event)=>{if(event.key.toLowerCase()==='h'&&!(event.target instanceof HTMLInputElement||event.target instanceof HTMLTextAreaElement))toggleHelp()})
