...
import { analyzeProject, sourceLines, type ProjectAnalysis, type SourceFileInput } from './domain/analyzer'
import './style.css'
import './analysis.css'
...
function renderGraph(graph: ArchitectureGraph) {
  ...
  graph.edges.forEach((edge)=>{const tube=new THREE.Mesh(new THREE.BufferGeometry(),new THREE.MeshBasicMaterial({color:'#86ffce',transparent:true,opacity:.5}));tube.userData={from:edge.from,to:edge.to};refreshEdge(tube);edgeMeshes.push(tube);group.add(tube)})
