...
if (!['partial', 'verified'].includes(readiness.categories.find((category) => category.id === 'compatibility')?.status)) throw new Error('compatibility status is invalid')
if (!['partial', 'verified'].includes(readiness.categories.find((category) => category.id === 'distribution')?.status)) throw new Error('distribution status is invalid')
